...
const EditProfile = ({status}) => {
    ...
    const handleRegisterOwner = (values) => {
        ...
        AccountService.registerOwner(data).then((response) => {
            ...
            handleSendNotify(account, 1, `${account.username} đã đăng ký làm chủ phòng`, 'profile/confirm-owner');
            navigate('/profile/information');
        }).catch(function (err) {
            console.log(err);
        })
    ...
    const handleTitle = () => {
        if (status) {
            ...
            return <h3 className="text-center mb-4 text-uppercase">Đơn đăng ký làm chủ phòng</h3>
        }
    }
    return (
        <div className="col-9">
